...
class TwoOnboardingModalViewController: UIViewController {
    
    private let centerCustomView = UIView()
    private let firstLabel = UILabel()
    private let secondOnboarding = UIImageView()
...
extension TwoOnboardingModalViewController {
    ...
    func constructSecondOnboarding() {
        secondOnboarding.image = UIImage(resource: .secondOnboarding)
        secondOnboarding.contentMode = .scaleAspectFill
        secondOnboarding.translatesAutoresizingMaskIntoConstraints = false
        centerCustomView.addSubview(secondOnboarding)
        
        NSLayoutConstraint.activate([
            secondOnboarding.topAnchor.constraint(equalTo: firstLabel.bottomAnchor, constant: 60),
            secondOnboarding.centerXAnchor.constraint(equalTo: centerCustomView.centerXAnchor),
            secondOnboarding.heightAnchor.constraint(equalToConstant: 250),
            secondOnboarding.widthAnchor.constraint(equalToConstant: 250)
        ])
    }
...
extension TwoOnboardingModalViewController {
    @objc func transitionCustomNextButton() {
        let threeOnboardingModalViewController = ThreeOnboardingModalViewController()
        navigationController?.pushViewController(threeOnboardingModalViewController, animated: true)
    }
